...
import { registerComputeResources, registerComputeTools } from "./services/compute/index.js";
import {
...
import { registerDiagramTools } from "./services/diagram/index.js";
...
// import { registerComputeTools } from "./services/compute/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    try {
      // Register Google Cloud Compute services
      logger.info("Registering Google Cloud Compute services");
      registerComputeResources(server);
      registerComputeTools(server);
    } catch (error) {
      logger.warn(
        `Error registering Compute services: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    try {
      ...
      logger.warn(
        `Error registering Diagram services: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
